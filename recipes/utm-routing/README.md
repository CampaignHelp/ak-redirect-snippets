...
Tested on Robotic Dogs (`roboticdogs.actionkit.com`) on 2026-04-22, 2026-05-15, and 2026-05-16 — all three branches (`if`/`elif`/`else`) pass via the automated Playwright matrix in `tests/`. Users arriving at `/signup/<your-page>/?src=<value>` have `{{ args.src }}` automatically available in the redirect URL template — no hidden form inputs needed on the page.

Note: one out of two runs on 2026-05-16 had the `if`/`elif` branches transiently fall through to `{% else %}` on Robotic Dogs (the same matrix passed cleanly minutes later with no code changes). The recipe logic was verified correct in isolation via a direct equality probe. Treat AK's redirect-template cache propagation as eventually-consistent rather than instant after a `followup.url` change — give it at least a few seconds before submitting a form against a freshly-installed snippet, and test repeatedly if the first submission gives an unexpected result.
